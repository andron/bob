...
# Check target sources, ordinary sources, mocable files and ui-forms. All files
# are real files, i.e. files that do exist and are not autogenerated by some
# other tool.
#
# Currently unused.
#
# $1: Target -- $2: Module name
# ------------------------------------------------------------------------------
define check_target_sources
$(call check_target_sources_srcs,$1,$2)
$(call check_target_sources_mocs,$1,$2)
...
define check_target_sources_srcs
$(if $(findstring undefined,$(origin $1_SRCS)),
	$(warning No _SRCS variable specified for target $1 in $($2_SRCDIR)$(RULES))
	$(error Empty _SRCS variable))
endef

define check_target_sources_mocs
$(if $(filter-out $(__srcs_mocs_filter),$($1_SRCS_MOC)),
	$(warning Moc sources should be header files, i.e. .h or .hh or .hpp)
	$(error Illegal _SRCS_MOC: $(filter-out $(__srcs_mocs_filter),$($1_SRCS_MOC))))
endef

define check_target_sources_missing
$(eval FILES := $(call __bob_dirprefix,$($2_SRCDIR),$(call __bob_filter_sources,$($1_SRCS))))
$(if $(call __bob_missing_files,$(FILES)),
	$(warning Missing sources for target $1 in $($2_SRCDIR)$(RULES))
	$(warning Missing file(s): $(call __bob_missing_files,$(FILES)))
	$(eval ERR_MISSING_FILES := yes))
endef

define check_target_sources_missing_mocs
$(eval FILES := $(call __bob_dirprefix,$($2_SRCDIR),$($1_SRCS_MOC)))
$(if $(call __bob_missing_files,$(FILES)),
	$(warning Missing moc-files for target $1 in $($2_SRCDIR)$(RULES))
	$(warning Missing file(s): $(call __bob_missing_files,$(FILES)))
	$(eval ERR_MISSING_FILES := yes))
endef

define check_target_sources_missing_uis
$(eval FILES := $(call __bob_dirprefix,$($2_SRCDIR),$($1_SRCS_FRM)))
$(if $(call __bob_missing_files,$(FILES)),
	$(warning Missing ui-files for target $1 in $($2_SRCDIR)$(RULES))
	$(warning Missing file(s): $(call __bob_missing_files,$(FILES)))
	$(eval ERR_MISSING_FILES := yes))
endef
